...
# Step 1: Connect to SQLite database (this will create 'company.db' file)
...
cursor.executemany('INSERT INTO DEPARTMENT (DEPARTMENT_ID, DEPARTMENT_NAME) VALUES (?, ?)', [
    (1, 'HR'),
    (2, 'Finance'),
    (3, 'Engineering'),
    (4, 'Sales'),
...
cursor.executemany('INSERT INTO EMPLOYEE (EMP_ID, FIRST_NAME, LAST_NAME, DOB, GENDER, DEPARTMENT) VALUES (?, ?, ?, ?, ?, ?)', [
...
cursor.executemany('INSERT INTO PAYMENTS (PAYMENT_ID, EMP_ID, AMOUNT, PAYMENT_TIME) VALUES (?, ?, ?, ?)', [
...
# Step 4: Save (commit) and close connection
conn.commit()
conn.close()
